...
const Card = ({ title, currentNumber, lastMonthNumber, historyData }) => {
  ...
  const getChartGradient = () => {
    return `url(#chartUv${currentNumber > lastMonthNumber ? 'Positive' : 'Negative'})`;
  };

  return (
    <div className='p-px bg-gradient-to-br from-[#6A6A9B] to-[#100F1D] rounded-lg overflow-clip w-full -z-40 relative h-full aspect-square sm:aspect-auto'>
      <div className="bg-primary text-textcolor p-4 md:p-5 relative overflow-hidden flex flex-col md:flex-row justify-between font-main w-full h-full rounded-[8px] md:rounded-lg">
